...
class RefDataset(Dataset):
    ...
    def __init__(self, data_root, split_root, dataset, split, mode, input_size,
        ...
        if self.dataset == 'mix':
            if split == 'train':
                self.dataset = 'gref_umd'
                dataset_path = osp.join(self.split_root, 'gref_umd')
                valid_splits = self.SUPPORTED_DATASETS[self.dataset]['splits']
                imgset_file = '{0}_{1}.pth'.format(self.dataset, 'train')
                imgset_path = osp.join(dataset_path, imgset_file)
                self.images += torch.load(imgset_path)

                self.dataset = 'gref'
                dataset_path = osp.join(self.split_root, 'gref')
                valid_splits = self.SUPPORTED_DATASETS[self.dataset]['splits']
                imgset_file = '{0}_{1}.pth'.format(self.dataset, 'train')
                imgset_path = osp.join(dataset_path, imgset_file)
                self.images += torch.load(imgset_path)
                ...
                self.dataset = 'unc+'
                dataset_path = osp.join(self.split_root, 'unc+')
                valid_splits = self.SUPPORTED_DATASETS[self.dataset]['splits']
                imgset_file = '{0}_{1}.pth'.format(self.dataset, 'train')
                imgset_path = osp.join(dataset_path, imgset_file)
                self.images += torch.load(imgset_path)
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            word_vec = tokenize(phrase, self.word_length, True).squeeze(0)
            return img, word_vec, np.array(bbox, dtype=np.float32), param
