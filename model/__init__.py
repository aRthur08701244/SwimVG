from .swimvg import SwimVG
from loguru import logger


def build_swimvg(args):
    model = SwimVG(args)
    backbone = []
    head = []
    neck = []
    ...
    for k, v in model.named_parameters():
        if (k.startswith('backbone') and 'positional_embedding' not in k or 'fusion' in k  or 'dinov2' in k) and v.requires_grad:
            backbone.append(v)
