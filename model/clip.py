...
class ResidualAttentionBlock(nn.Module):
    def __init__(self,
        ...
        if use_adapter:
            self.adapter = TextAdapter(d_model, txt_adapter_dim) #fc_in_channels, in_channels
        self.use_prompt = use_prompt
        if use_prompt:
            self.prompts = nn.Parameter(torch.zeros(1, 1, d_model))
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.use_prompt:
            ...
            x = torch.cat([x,prompts.permute(1,0,2)],dim = 0)
...
class CLIP(nn.Module):
    def __init__(
        ...
        self.transformer = Transformer(
            ...
            attn_mask=self.build_attention_mask(txt_length+1),
        ...
        self.prompt = torch.zeros(1, transformer_width)
